...
async def lifespan(app: FastAPI):
    ...
    rec_store.load(
        ...
        columns=["user_id", "track_id", "final_rank_score"],
    )
    rec_store.load(
        "default",
        'top_popular.parquet', 
        columns=["track_id", "interaction_count"],
    )
    logger.info("Ready!")
    yield
...
async def recommendations_default(k: int = 10):
    """
...
async def recommendations_online(user_id: int, k: int = 10):
    ...
    for track_id in events:
        ...
        items += item_similar_items["similar_track_id"]
        scores += item_similar_items["similarity_score"]
